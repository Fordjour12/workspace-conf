...
# =====================================================================
# Google Cloud Run Deployment Setup Script (FINAL)
# FIX: Added Service Account Token Creator binding for Docker auth.
# =====================================================================

# Check if run.sh exists in current directory
if [ ! -f "run.sh" ]; then
    echo "ERROR: run.sh not found in current directory."
    echo "Usage: $0 (expects run.sh in current directory)"
    exit 1
fi

# Source the run.sh file
echo "Loading configuration from: run.sh"
source run.sh
...
export ROLE_SERVICE_ACCOUNT_TOKEN_CREATOR="roles/iam.serviceAccountTokenCreator" # <-- NEW: Role for token creation
...
if [ -n "${GCS_BUCKET_NAME:-}" ]; then
    echo "Storage Bucket: ${GCS_BUCKET_NAME}"
fi
echo ""
echo "Press Enter to continue with deployment setup, or Ctrl+C to cancel..."
read -r
echo ""
...
# --- 3. Assign IAM Roles ---
echo "[3] Assigning roles to service account at the Project level..."
for ROLE in \
    "${ROLE_RUN_ADMIN}" \
...
# Define the principal member for WIF bindings
...
# 5a. Bind service account to WIF provider (Workload Identity User)
echo "  - Binding Workload Identity User role (for impersonation)..."
gcloud iam service-accounts add-iam-policy-binding "${SA_EMAIL}" \
    --project="${GCP_PROJECT_ID}" \
    --role="${ROLE_WORKLOAD_IDENTITY_USER}" \
    --member="${WIF_MEMBER}" --quiet

# 5b. FIX: Bind Service Account Token Creator role (for generating access tokens)
echo "  - Binding Service Account Token Creator role (for 'getAccessToken' fix)..."
gcloud iam service-accounts add-iam-policy-binding "${SA_EMAIL}" \
    --project="${GCP_PROJECT_ID}" \
    --role="${ROLE_SERVICE_ACCOUNT_TOKEN_CREATOR}" \
    --member="${WIF_MEMBER}" --quiet
echo "  WIF principal now has Service Account Token Creator role."
...
echo "----------------------------------------"
echo ""
echo "Please run this script again to apply the changes, and then re-run your GitHub Actions workflow."
